...
- รับ/ส่งหัวใจจากกล่องจดหมายด้วยปุ่มแยก กด Confirm สีเขียวซ้ำจนครบทุกรายการแล้วปิดกล่องเอง
- ปิดป๊อปอัปอีเวนท์/ประกาศที่กด X ไม่โดนตำแหน่งเดิมได้เอง (ไล่กดหลายจุด + BACK + รีสตาร์ทเกม) และบันทึกภาพตอนค้างลง `debug_screens`
...
## รับ/ส่งหัวใจจากกล่องจดหมาย

1. เปิดหน้า **Friends** (เห็นปุ่มกล่องจดหมายที่มุมขวาของรายชื่อ)
2. กดปุ่ม **รับหัวใจจากกล่องจดหมาย** ในแถบ RUN CONTROL (ปุ่มสีน้ำเงิน ถัดจาก "ส่งหัวใจ")
3. บอทจะเปิดกล่องจดหมาย → เปิดแท็บ Lives → กดรับทั้งหมด แล้วไล่กด Confirm สีเขียวซ้ำ
   จนกว่าจะครบทุกรายการ จากนั้นกดปิดกล่องและกลับมาที่หน้า Friends
4. ถ้าไม่มีหัวใจให้รับในกล่อง บอทจะปิดกล่องทันทีโดยไม่กดปุ่มใดเพิ่ม

โหมดนี้ทำงานเป็นกระบวนการแยก (ไม่เริ่มรอบ ไม่ซื้อไอเทม) และปลอดภัยแบบเดียวกับ "ส่งหัวใจ":
ตรวจหน้าจอทุกครั้งก่อนกด มีขีดจำกัดรอบและกันการกดซ้ำตำแหน่งเดิม หากหน้าจอเปลี่ยนไปโดยไม่คาดคิด
บอทจะหยุดทันทีและบันทึกภาพหน้าจอลงโฟลเดอร์ `debug_screens` เพื่อใช้ปรับปรุงภายหลัง

## ป๊อปอัปอีเวนท์/ประกาศที่ปิดไม่ได้

หน้ายืนยัน/ประกาศบางแบบมีปุ่ม X อยู่คนละตำแหน่งตามขนาดหน้าต่าง เช่น
**หน้าต่าง News (ประกาศข่าว/อัปเดต)** ใช้ปุ่ม X วงกลมมุมขวาบนที่ `(1125, 55)`
ซึ่งไม่ใช่ตำแหน่งเดียวกับประกาศแบบอื่น บอทรุ่นนี้จะจำหน้า News ได้เองแล้วกด X ปิดถูกจุด
ส่วนป๊อปอัปอื่นจะไล่กดปุ่ม X หลายตำแหน่งที่เคยพบ แล้วกด BACK เป็นตัวสำรอง
หากยังปิดไม่ได้ บอทจะบันทึกภาพหน้าจอลง `debug_screens` แล้วรีสตาร์ทเกมเพื่อกลับมาทำงานต่อเอง
ไม่ต้องกดปุ่มใดด้วยมือระหว่างที่บอทค้างอยู่
